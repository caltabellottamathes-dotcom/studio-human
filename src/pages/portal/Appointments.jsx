...
const aptTypeLabel = { intake: 'Intakegesprek', session: 'Sessie', online: 'Online sessie', physical: 'Fysieke sessie', phone: 'Telefoongesprek' };
  ...
  const AppointmentCard = ({ a }) => (
    <div className="bg-white rounded-xl border border-neutral-200 p-5 flex items-start gap-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-2">
          {a.status === 'scheduled' && <span className="text-[9px] uppercase tracking-widest px-2 py-0.5 rounded-full bg-blue-50 text-blue-600">Gepland</span>}
          {a.status === 'completed' && <span className="text-[9px] uppercase tracking-widest px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 flex items-center gap-1"><Check className="w-3 h-3" /> Voltooid</span>}
          {a.status === 'cancelled' && <span className="text-[9px] uppercase tracking-widest px-2 py-0.5 rounded-full bg-neutral-100 text-neutral-400 flex items-center gap-1"><X className="w-3 h-3" /> Geannuleerd</span>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="p-6 md:p-10 max-w-3xl">
      <div className="mb-8">
        ...
        <h1 className="font-display text-3xl md:text-4xl text-neutral-800 tracking-tight">Afspraken</h1>
        <p className="text-neutral-500 text-sm font-light mt-2">Overzicht van je geplande en afgelopen afspraken.</p>
      </div>

      {/* Upcoming */}
      <div className="mb-8">
        <h2 className="font-display text-lg text-neutral-800 mb-4 flex items-center gap-2">
          <Calendar className="w-4 h-4 text-red-600" strokeWidth={1.5} /> Aankomend
        </h2>
        {upcoming.length === 0 ? (
          <div className="bg-white rounded-xl border border-neutral-200 p-8 text-center">
            <p className="text-sm text-neutral-400 font-light">Geen aankomende afspraken.</p>
          </div>
        ) : (
