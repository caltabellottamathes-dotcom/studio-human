...
const tabs = [
  ...
  { id: 'notities', label: 'Sessienotities', icon: FileText },
  ...
  { id: 'stemming', label: 'Stemming', icon: Heart },
];
  ...
  return (
    <div className="p-6 md:p-10 max-w-5xl">
      ...
      <Link to="/admin/clienten" className="inline-flex items-center gap-2 text-xs uppercase tracking-widest text-neutral-500 hover:text-red-600 transition-colors mb-6">
        <ArrowLeft className="w-4 h-4" /> Terug naar cliënten
      </Link>
        ...
        <div>
          <div className="flex items-center gap-3 mb-1">
            <h1 className="font-display text-3xl md:text-4xl text-neutral-800 tracking-tight">
              {profile?.first_name || data?.user?.full_name || 'Cliënt'} {profile?.last_name || ''}
            </h1>
            ...
            }`}>{profile?.status === 'active' ? 'Actief' : profile?.status === 'archived' ? 'Gearchiveerd' : 'In afwachting'}</span>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl border border-neutral-200 p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><Label className="text-xs uppercase tracking-widest text-neutral-500">Voornaam</Label><Input value={profileForm.first_name || ''} onChange={e => set('first_name', e.target.value)} className="mt-1" /></div>
              <div><Label className="text-xs uppercase tracking-widest text-neutral-500">Achternaam</Label><Input value={profileForm.last_name || ''} onChange={e => set('last_name', e.target.value)} className="mt-1" /></div>
              <div><Label className="text-xs uppercase tracking-widest text-neutral-500">Geboortedatum</Label><Input type="date" value={profileForm.date_of_birth || ''} onChange={e => set('date_of_birth', e.target.value)} className="mt-1" /></div>
              <div><Label className="text-xs uppercase tracking-widest text-neutral-500">Telefoon</Label><Input value={profileForm.phone || ''} onChange={e => set('phone', e.target.value)} className="mt-1" /></div>
              <div className="col-span-2"><Label className="text-xs uppercase tracking-widest text-neutral-500">Adres</Label><Input value={profileForm.address || ''} onChange={e => set('address', e.target.value)} className="mt-1" /></div>
              <div><Label className="text-xs uppercase tracking-widest text-neutral-500">Postcode</Label><Input value={profileForm.postal_code || ''} onChange={e => set('postal_code', e.target.value)} className="mt-1" /></div>
              <div><Label className="text-xs uppercase tracking-widest text-neutral-500">Stad</Label><Input value={profileForm.city || ''} onChange={e => set('city', e.target.value)} className="mt-1" /></div>
            </div>
          </div>
          <div className="bg-white rounded-xl border border-neutral-200 p-6">
            <h2 className="font-display text-lg text-neutral-800 mb-4">Noodcontact</h2>
            <div className="grid grid-cols-2 gap-4">
              <div><Label className="text-xs uppercase tracking-widest text-neutral-500">Naam</Label><Input value={profileForm.emergency_contact_name || ''} onChange={e => set('emergency_contact_name', e.target.value)} className="mt-1" /></div>
              <div><Label className="text-xs uppercase tracking-widest text-neutral-500">Telefoon</Label><Input value={profileForm.emergency_contact_phone || ''} onChange={e => set('emergency_contact_phone', e.target.value)} className="mt-1" /></div>
              <div><Label className="text-xs uppercase tracking-widest text-neutral-500">Relatie</Label><Input value={profileForm.emergency_contact_relation || ''} onChange={e => set('emergency_contact_relation', e.target.value)} className="mt-1" /></div>
            </div>
          </div>
          <div className="bg-red-50/40 rounded-xl border border-red-100 p-6">
            <div className="flex items-center gap-2 mb-4">
              <Lock className="w-4 h-4 text-red-600" />
              <h2 className="font-display text-lg text-neutral-800">Privénotities</h2>
            </div>
            <Textarea value={profileForm.admin_notes || ''} onChange={e => set('admin_notes', e.target.value)} rows={4} placeholder="Privé notities over deze cliënt — niet zichtbaar voor de cliënt" />
          </div>
      ...
      {activeTab === 'afspraken' && (
        <div>
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-display text-lg text-neutral-800">Afspraken ({data?.appointments?.length || 0})</h2>
          ...
          {(data?.appointments || []).length === 0 ? (
            <p className="text-sm text-neutral-400 font-light py-8 text-center">Nog geen afspraken.</p>
          ) : (
            <div className="space-y-2">
              {data.appointments.map(a => (
                <div key={a.id} className="bg-white rounded-lg border border-neutral-200 p-4 flex items-center justify-between">
                  ...
                  }`}>{a.status === 'scheduled' ? 'Gepland' : a.status === 'completed' ? 'Voltooid' : a.status === 'cancelled' ? 'Geannuleerd' : a.status}</span>
                </div>
              ))}
            </div>
          )}
      ...
      {activeTab === 'notities' && (
        <div>
          <div className="flex justify-between items-center mb-4">
            <div>
              <h2 className="font-display text-lg text-neutral-800">Sessienotities ({data?.sessionNotes?.length || 0})</h2>
              <p className="text-xs text-red-600/70 uppercase tracking-widest mt-1 flex items-center gap-1"><Lock className="w-3 h-3" /> Privé — niet zichtbaar voor cliënt</p>
            </div>
            <button onClick={() => setDialog('note')} className="inline-flex items-center gap-2 px-4 py-2 bg-neutral-900 hover:bg-black text-white rounded-full text-xs uppercase tracking-widest">
              <Plus className="w-3.5 h-3.5" /> Nieuwe notitie
            </button>
          </div>
          {(data?.sessionNotes || []).length === 0 ? (
            <p className="text-sm text-neutral-400 font-light py-8 text-center">Nog geen sessienotities.</p>
          ) : (
            <div className="space-y-3">
              {data.sessionNotes.map(n => (
                <div key={n.id} className="bg-white rounded-lg border border-neutral-200 p-5">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-sm text-neutral-800 font-medium">{new Date(n.session_date).toLocaleDateString('nl-NL', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p>
                  </div>
                  {n.summary && <p className="text-sm text-neutral-600 font-light mb-2">{n.summary}</p>}
                  {n.observations && <p className="text-xs text-neutral-500 mt-2"><span className="uppercase tracking-widest text-neutral-400">Observaties: </span>{n.observations}</p>}
                  {n.risk_assessment && <p className="text-xs text-red-600 mt-2"><span className="uppercase tracking-widest">Risico: </span>{n.risk_assessment}</p>}
                </div>
              ))}
            </div>
          )}
      ...
      {activeTab === 'opdrachten' && (
        <div>
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-display text-lg text-neutral-800">Opdrachten ({data?.assignments?.length || 0})</h2>
            <button onClick={() => setDialog('assignment')} className="inline-flex items-center gap-2 px-4 py-2 bg-neutral-900 hover:bg-black text-white rounded-full text-xs uppercase tracking-widest">
              <Plus className="w-3.5 h-3.5" /> Nieuwe opdracht
            </button>
          </div>
          {(data?.assignments || []).length === 0 ? (
            <p className="text-sm text-neutral-400 font-light py-8 text-center">Nog geen opdrachten.</p>
          ) : (
            <div className="space-y-2">
              {data.assignments.map(a => (
                <div key={a.id} className="bg-white rounded-lg border border-neutral-200 p-4">
                  <div className="flex items-center justify-between">
                    ...
                    }`}>{a.status === 'assigned' ? 'Toegewezen' : a.status === 'submitted' ? 'Ingediend' : a.status === 'reviewed' ? 'Beoordeeld' : a.status}</span>
                  </div>
      ...
      {activeTab === 'documenten' && (
        <div>
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-display text-lg text-neutral-800">Documenten ({data?.documents?.length || 0})</h2>
            <button onClick={() => setDialog('document')} className="inline-flex items-center gap-2 px-4 py-2 bg-neutral-900 hover:bg-black text-white rounded-full text-xs uppercase tracking-widest">
              <Plus className="w-3.5 h-3.5" /> Document delen
            </button>
          </div>
          {(data?.documents || []).length === 0 ? (
            <p className="text-sm text-neutral-400 font-light py-8 text-center">Nog geen documenten gedeeld.</p>
          ) : (
            <div className="space-y-2">
      ...
      {activeTab === 'berichten' && (
        <div>
          <h2 className="font-display text-lg text-neutral-800 mb-4">Berichten</h2>
          <div className="bg-white rounded-xl border border-neutral-200 p-4 mb-4 max-h-[400px] overflow-y-auto space-y-3">
            {(data?.messages || []).length === 0 ? (
              <p className="text-sm text-neutral-400 font-light py-8 text-center">Nog geen berichten.</p>
            ) : (
              data.messages.map(m => (
          ...
          <div className="flex gap-2">
            <Textarea value={messageText} onChange={e => setMessageText(e.target.value)} placeholder="Typ een bericht..." rows={2} className="flex-1" onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(); } }} />
      ...
      {activeTab === 'stemming' && (
        <div>
          <h2 className="font-display text-lg text-neutral-800 mb-4">Stemming ({data?.moodEntries?.length || 0})</h2>
          {(data?.moodEntries || []).length === 0 ? (
            <p className="text-sm text-neutral-400 font-light py-8 text-center">Nog geen stemmingen geregistreerd.</p>
          ) : (
            <div className="bg-white rounded-xl border border-neutral-200 p-6">
              ...
              <div className="space-y-2 mt-4">
                {data.moodEntries.slice(0, 5).map((m, i) => (
                  <div key={i} className="flex items-center justify-between py-2 border-b border-neutral-100 last:border-0">
                    <span className="text-xs text-neutral-500">{new Date(m.entry_date).toLocaleDateString('nl-NL')}</span>
