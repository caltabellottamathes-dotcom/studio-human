  ...
  return (
    <div className="p-6 md:p-10 max-w-4xl">
      <div className="mb-8">
        ...
        <h1 className="font-display text-3xl md:text-4xl text-neutral-800 tracking-tight">Sessienotities</h1>
        <p className="text-xs text-red-600/70 uppercase tracking-widest mt-2 flex items-center gap-1"><Lock className="w-3 h-3" /> Privé — niet zichtbaar voor cliënten</p>
      </div>

      {notes.length === 0 ? (
        <div className="bg-white rounded-xl border border-neutral-200 p-12 text-center">
          <p className="text-sm text-neutral-400 font-light">Nog geen sessienotities.</p>
          <p className="text-xs text-neutral-400 mt-2">Open een cliënt om een notitie toe te voegen.</p>
        </div>
      ) : (
          ...
          {notes.map(n => (
            <Link key={n.id} to={`/admin/clienten/${n.client_id}`} className="bg-white rounded-lg border border-neutral-200 p-5 block hover:border-neutral-300 transition-colors group">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-neutral-400">{new Date(n.session_date).toLocaleDateString('nl-NL', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
              </div>
              ...
              <div className="flex items-center gap-1 text-[10px] uppercase tracking-widest text-red-600 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                Openen <ArrowRight className="w-3 h-3" />
              </div>
            </Link>
          ))}
        </div>
