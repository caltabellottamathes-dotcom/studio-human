    ...
    >
      <div className="max-w-[120rem] mx-auto px-6 md:px-12 py-6 md:py-14">
        <div className="mb-6 md:mb-10">
          <span className="text-xs uppercase tracking-[0.25em] text-red-600/80 block label-line mb-4 font-medium">amorvitae.</span>
          <p className="font-display text-xl md:text-2xl text-neutral-800 leading-[1.1] tracking-tight max-w-[24ch]">
            Liefde voor het leven, <span className="italic font-light text-red-600/90">zorg voor de ziel</span>.
          </p>
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3 md:gap-6">
          <nav className="flex flex-col gap-1 text-sm text-neutral-500 font-light w-fit" aria-label="Footer navigatie">
            {navLinks.map(link => (
          ...
          <div className="flex flex-col gap-1 text-sm text-neutral-500 font-light md:text-right">
            <p className="text-neutral-700">De Gaer 8, Hasselt</p>
            <a href="tel:+32476376675" className="hover:text-red-600 transition-colors py-1">+32 476 37 66 75</a>
            <a href="mailto:debora@amorvitae.be" className="hover:text-red-600 transition-colors py-1">debora@amorvitae.be</a>
          </div>
        </div>

        <div className="mt-6 pt-4 md:mt-8 border-t border-neutral-200/60 flex items-center justify-between">
          <span className="text-[10px] text-neutral-400 uppercase tracking-widest">© {new Date().getFullYear()} Amor Vitae</span>
          <Link to="/admin/dashboard" className="text-[10px] text-neutral-400 hover:text-red-600 transition-colors uppercase tracking-widest">Beheerder</Link>
        </div>
      </div>
      <AssessmentLightbox />
    </motion.footer>
