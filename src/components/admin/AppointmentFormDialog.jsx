  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="font-display text-lg">New appointment</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <Label className="text-xs uppercase tracking-widest text-neutral-500">Time</Label>
              <Input type="time" value={form.start_time} onChange={e => set('start_time', e.target.value)} className="mt-1" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label className="text-xs uppercase tracking-widest text-neutral-500">Duration (min)</Label>
              <Input type="number" value={form.duration_minutes} onChange={e => set('duration_minutes', parseInt(e.target.value) || 60)} className="mt-1" />
            </div>
            <div>
              <Label className="text-xs uppercase tracking-widest text-neutral-500">Price ($)</Label>
          ...
          <div>
            ...
            <Input value={form.location} onChange={e => set('location', e.target.value)} placeholder="Practice / Online / ..." className="mt-1" />
          </div>
          <div>
            <Label className="text-xs uppercase tracking-widest text-neutral-500">Note for client</Label>
