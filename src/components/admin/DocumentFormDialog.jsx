  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="font-display text-lg">Document delen</DialogTitle>
        </DialogHeader>
        {error && <p className="text-sm text-red-600">{error}</p>}
        <div className="space-y-4">
          <div>
            <Label className="text-xs uppercase tracking-widest text-neutral-500">Titel</Label>
            <Input value={form.title} onChange={e => set('title', e.target.value)} placeholder="bijv. Intakeverslag" className="mt-1" />
          </div>
          <div>
            <Label className="text-xs uppercase tracking-widest text-neutral-500">Categorie</Label>
            <select value={form.category} onChange={e => set('category', e.target.value)} className="w-full mt-1 border border-neutral-200 rounded-md px-3 py-2 text-sm bg-white">
              ...
              <option value="report">Verslag</option>
          ...
          <div>
            <Label className="text-xs uppercase tracking-widest text-neutral-500">Bestand</Label>
            <div className="mt-1 border-2 border-dashed border-neutral-200 rounded-lg p-4 text-center">
              {fileUrl ? (
                <p className="text-sm text-emerald-600">✓ Bestand geüpload</p>
              ) : uploading ? (
                ...
                <label className="cursor-pointer">
                  ...
                  <span className="text-xs text-neutral-500">Klik om te uploaden</span>
